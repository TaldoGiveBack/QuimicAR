...
public final class ShaderUtil {
  ...
  public static int loadGLShader(
    ...
    if (compileStatus[0] == 0) {
      Log.e(tag, "Erro ao compilar shader: " + GLES20.glGetShaderInfoLog(shader));
  ...
  public static void checkGLError(String tag, String label) {
    ...
    while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
      Log.e(tag, label + ": erroGL " + error);
      lastError = error;
    }
    if (lastError != GLES20.GL_NO_ERROR) {
      throw new RuntimeException(label + ": erroGL " + lastError);
    }
  }
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (tokens[0].equals("#include")) {
          ...
          if (includeFilename.equals(filename)) {
            throw new IOException("Não inclua o arquivo que chama.");
          }
